...
function createTestUser(knex) {
  return modelUtil.getHash(user.password).then(hash => {
    return knex('users').insert(modelUtil.addTimestamps(knex, {
      email: user.email,
      password: hash,
...
function deleteTestUser(knex, email, deleteContentOnly = false) {
  ...
  return knex('users').where('email', email).first('id').then(testUser => {
    ...
    const tablesToDeleteFrom = ['section_orders', 'sections', 'plan_orders', 'plans', 'chapter_topics', 'master_topic_orders', 'master_topics', 'chapter_orders', 'chapters', 'document_orders', 'documents', 'workshop_content'];
...
function setTestUserResetKey(knex) {
  return modelUtil.getHash(user.resetKey).then(hash => {
    return knex('users').where('email', user.email).update({
      'pass_reset_key': hash
    });
  });
...
exports.createAlternateTestUser = createAlternateTestUser;
exports.createTestDocument = createTestDocument;
exports.createTestChapter = createTestChapter;
exports.deleteTestUser = deleteTestUser;
exports.getDocuments = getDocuments;
exports.getTestUserId = getTestUserId;
exports.makeTestUserAdmin = makeTestUserAdmin;
exports.makeTestUserDemo = makeTestUserDemo;
exports.makeTestUserPremium = makeTestUserPremium;
exports.route = route;
exports.setTestUserPaymentDueDate = setTestUserPaymentDueDate;
exports.setTestUserResetKey = setTestUserResetKey;
exports.setTestUserStripeId = setTestUserStripeId;
exports.setTestUserVerifyKey = setTestUserVerifyKey;
exports.stubRecaptcha = stubRecaptcha;
exports.workshopGuids = workshopGuids;
